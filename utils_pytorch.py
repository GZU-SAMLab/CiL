...
def generate_etf_vector(in_channels, num_classes, dataset_name=''):
    """生成等距紧框架 (ETF) 向量 - 已替换为CBSE方法
    
    参数:
        in_channels: 特征维度（512）
        num_classes: 类别数
        dataset_name: 数据集名称（用于自动选择CBSE配置）
    """
    # 使用CBSE方法替代传统ETF
    from utils_incremental.cbse import generate_cbse_anchor
    
    # 生成CBSE稀疏向量
    cbse_vec = generate_cbse_anchor(in_channels, num_classes, dataset_name=dataset_name, seed=42)
